...
class AstNode:
    ...
    def from_dict(cls, data: dict) -> Self:
        cls_dict = dict()
        ty_hints = {{k:v for k,v in get_type_hints(cls).items() if not k.startswith("_")}}
        ...
        for field, ty in ty_hints.items():
            ...
            if ty in cls._PRIMITIVE_TYPES:
                cls_dict[original_field] = ty(data[field])

            elif get_origin(ty) is list:
                field_data = data[field]
                cls_dict[original_field] = [
                    cls._instantiate_typed_field(el, get_args(ty)[0])
    ...
    @classmethod
    def _instantiate_typed_field(cls, data: dict, ty: typing.Type[Any]):
        args = get_args(ty)

        if not args:
            # Initialize this specific class
            return ty.from_dict(data)
        ...
        if args[0] in cls._PRIMITIVE_TYPES:
            ...
            return [cls._instantiate_typed_field(el, get_args(args[0])[0]) for el in data]
